...
$forbiddenBinaryPatterns = @('DIAG[-_][0-9]{4}', '\b(?:EVD|FND|ATT|HYP)-[A-Z0-9]', '\b(?:META-)?METHOD-[0-9]{3}\b')
foreach ($pattern in $forbiddenBinaryPatterns) { if ([regex]::IsMatch($elfText, $pattern)) { throw "Release ELF contains forbidden pattern: $pattern" } }

$packageReport = Get-Content -Raw -LiteralPath (Join-Path $PackageRoot "package-report.json") | ConvertFrom-Json
if ($packageReport.status -ne "PASS" -or $packageReport.version -ne $ExpectedVersion -or $packageReport.program_id -ne $ProgramId -or $packageReport.competition_hud -ne "omitted" -or $packageReport.competition_enabled) { throw "Package report failed release identity checks." }
...
if ($manifest.version -ne $ExpectedVersion -or $manifest.program_id -ne $ProgramId -or $manifest.exlaunch_commit -ne $ExpectedCommit -or $manifest.competition_hud -ne "omitted" -or $manifest.competition_enabled) { throw "Expanded package manifest failed identity checks." }
...
if (@($expandedFiles | Where-Object { $_.Extension -in @('.szs', '.xdelta') }).Count -ne 0) { throw "Initial alpha package must not contain HUD archives or patches." }
...
$report = [ordered]@{
    ...
    ci_workflow_pins = "PASS"; ci_equivalent_public_tree_build = "PASS"; hosted_actions_execution = "pending Gate G repository staging"
    zip_sha256 = [string]$packageReport.zip.sha256; package_file_count = $expandedFiles.Count
    hud_archive_present = $false; competition_enabled = $false
